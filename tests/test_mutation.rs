...
fn test_structural_mutation_behavior() {
    ...
    let cfg_forced_add = MutateConfig {
        sigma: 0.1,
        prob: 1.0,
        reset_chance: 0.1,
    ...
    // Config for forced mutation
    let cfg_forced_delete = MutateConfig {
        sigma: 0.1,
        prob: 1.0,
        reset_chance: 0.1,
        insert_layer_chance: 0.0,
        add_neuron_chance: 0.0,
        delete_neuron_chance: 1.0,
        clip: Some((-1.0, 1.0)),
        min_neurons: 2,
    };
    ...
    let mutated_forced = net_empty.mutate_using(&mut rng, &cfg_forced_add);
    assert!(
    ...
    // -------- Case 2: Check neuron expansion in forced add config
    let net_single = NeuralNetwork::new(input_size, output_size, vec![2]);
    let mutated_neurons = net_single.mutate_using(&mut rng, &cfg_forced_add);
    assert!(
        mutated_neurons
    ...
    // -------- Case 3: Check neuron expansion in forced delete config
    let net_single_2 = NeuralNetwork::new(input_size, output_size, vec![3]);
    println!("{:?}", net_single_2.get_layer_output_sizes());
    let mutated_neurons = net_single_2.mutate_using(&mut rng, &cfg_forced_delete);
    assert!(
        mutated_neurons.get_layer_output_sizes()[0] == 2,
        "A node should have been deleted: {:?}",
        mutated_neurons.get_layer_output_sizes()
    );

    // -------- Case 4: Probabilistic mutation eventually adds structure
